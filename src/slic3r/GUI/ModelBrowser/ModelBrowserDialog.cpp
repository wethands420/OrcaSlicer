...
ModelBrowserPanel::ModelBrowserPanel(wxWindow* parent, Plater* plater)
    ...
    , m_plater(plater)
{
    ...
    if (m_browser == nullptr) {
        ...
        m_browser->Bind(wxEVT_WEBVIEW_NAVIGATING, &ModelBrowserPanel::on_navigation_request, this);
        m_browser->Bind(wxEVT_WEBVIEW_NAVIGATED, &ModelBrowserPanel::on_navigation_complete, this);
        m_browser->Bind(wxEVT_WEBVIEW_LOADED, &ModelBrowserPanel::on_document_loaded, this);
        m_browser->Bind(wxEVT_WEBVIEW_NEWWINDOW, &ModelBrowserPanel::on_new_window, this);
        m_browser->Bind(wxEVT_WEBVIEW_SCRIPT_MESSAGE_RECEIVED, &ModelBrowserPanel::on_script_message, this);
#ifdef __WIN32__
    ...
    Bind(EVT_DWNLDR_FILE_PROGRESS, &ModelBrowserPanel::on_download_progress, this);
    Bind(EVT_DWNLDR_FILE_COMPLETE, &ModelBrowserPanel::on_download_complete, this);
    Bind(EVT_DWNLDR_FILE_ERROR, &ModelBrowserPanel::on_download_error, this);
...
void ModelBrowserPanel::build_toolbar(wxBoxSizer* parent_sizer)
{
    ...
    external->Bind(wxEVT_BUTTON, &ModelBrowserPanel::on_open_external, this);
    download->Bind(wxEVT_BUTTON, &ModelBrowserPanel::on_download_current, this);
    download_folder_button->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) { choose_download_folder(); });
}

void ModelBrowserPanel::load_selected_site()
{
...
void ModelBrowserPanel::load_url(const wxString& url)
{
...
void ModelBrowserPanel::open_external_browser()
{
...
boost::filesystem::path ModelBrowserPanel::download_folder() const
{
...
void ModelBrowserPanel::choose_download_folder()
{
...
void ModelBrowserPanel::on_site_selected(wxCommandEvent& /*event*/)
{
    load_selected_site();
}

void ModelBrowserPanel::on_address_enter(wxCommandEvent& /*event*/)
{
    load_url(m_address->GetValue());
}

void ModelBrowserPanel::on_back(wxCommandEvent& /*event*/)
{
    if (m_browser && m_browser->CanGoBack())
        m_browser->GoBack();
}

void ModelBrowserPanel::on_forward(wxCommandEvent& /*event*/)
{
    if (m_browser && m_browser->CanGoForward())
        m_browser->GoForward();
}

void ModelBrowserPanel::on_reload(wxCommandEvent& /*event*/)
{
    if (m_browser)
        m_browser->Reload();
}
...
void ModelBrowserPanel::on_download_current(wxCommandEvent& /*event*/)
{
...
void ModelBrowserPanel::on_navigation_request(wxWebViewEvent& event)
{
...
void ModelBrowserPanel::on_navigation_complete(wxWebViewEvent& event)
{
    m_address->ChangeValue(event.GetURL());
    event.Skip();
}

void ModelBrowserPanel::on_new_window(wxWebViewEvent& event)
{
    const wxString url = event.GetURL();
    event.Veto();
    if (is_http_url(url) && maybe_handle_download_request(url)) {
...
void ModelBrowserPanel::on_document_loaded(wxWebViewEvent& event)
{
#ifdef __WIN32__
...
void ModelBrowserPanel::on_script_message(wxWebViewEvent& event)
{
...
bool ModelBrowserPanel::maybe_handle_download_request(const wxString& url, const wxString& suggested_filename)
{
    if (!is_http_url(url))
        return false;
...
void ModelBrowserPanel::inject_page_bridge()
{
    if (!m_browser)
        return;
...
void ModelBrowserPanel::start_download(const wxString& url, const wxString& suggested_filename)
{
    if (m_active_download) {
...
void ModelBrowserPanel::on_download_progress(wxCommandEvent& event)
{
    m_status->SetLabel(format_wxstr(_L("Downloading model file... %1%"), event.GetString() + "%"));
}

void ModelBrowserPanel::on_download_complete(wxCommandEvent& event)
{
...
void ModelBrowserPanel::on_download_error(wxCommandEvent& event)
{
    m_active_download.reset();
...
void ModelBrowserPanel::handle_downloaded_file(const boost::filesystem::path& path)
{
...
void ModelBrowserPanel::import_geometry(const boost::filesystem::path& path)
{
    if (!m_plater)
        return;
...
void ModelBrowserPanel::open_project(const boost::filesystem::path& path)
{
    if (!m_plater)
        return;
...
void ModelBrowserPanel::inspect_zip_archive(const boost::filesystem::path& path)
{
    if (!m_plater)
        return;
    wxGetApp().mainframe->select_tab(MainFrame::tp3DEditor);
...
void ModelBrowserPanel::save_file_as(const boost::filesystem::path& path)
{
...
#ifdef __WIN32__
struct ModelBrowserPanel::NativeDownload
{
...
void ModelBrowserPanel::install_native_download_interceptor()
{
...
void ModelBrowserPanel::remove_native_download_interceptor()
{
    if (!m_browser || !m_native_download_interceptor_installed)
        return;
